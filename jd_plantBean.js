...
async function jdPlantBean() {
  try {
    ...
    if (
      ...
      subTitle =
        `【京东昵称】${$.plantBeanIndexResult.data.plantUserInfo.plantNickName}`;
...
async function doGetReward() {
  ...
  if (roundList[num].dateDesc.indexOf("本期 ") > -1) {
    roundList[num].dateDesc = roundList[num].dateDesc.substr(
      roundList[num].dateDesc.indexOf("本期 ") + 3,
...
async function doCultureBean() {
  ...
  if ($.plantBeanIndexResult && $.plantBeanIndexResult.code === "0") {
    ...
    console.log(
      `plantBeanIndexResult:${JSON.stringify($.plantBeanIndexResult)}`,
    );
  }
}
async function stealFriendWater() {
  ...
  if ($.stealFriendList && $.stealFriendList.code === "0") {
    ...
    if (
      $.stealFriendList.data && $.stealFriendList.data.friendInfoList &&
      $.stealFriendList.data.friendInfoList.length > 0
    ) {
  ...
  if ($.taskList && $.taskList.length > 0) {
    for (let item of $.taskList) {
      ...
      if (item.dailyTimes === 1 && item.taskType !== 8) {
        ...
        console.log(
          `做 ${item.taskName}任务结果:${
            JSON.stringify($.receiveNutrientsTaskRes)
          }\n`,
        );
      }
      if (item.taskType === 3) {
        ...
        for (let shop of shopList) {
          ...
          if (shopRes && shopRes.code === "0") {
            if (
              shopRes.data && shopRes.data.nutrState &&
              shopRes.data.nutrState === "1"
            ) {
              unFinishedShopNum--;
            }
          }
      ...
      if (item.taskType === 5) {
        ...
        for (let product of productList) {
          ...
          if (productRes && productRes.code === "0") {
            ...
            if (
              productRes.data && productRes.data.nutrState &&
              productRes.data.nutrState === "1"
            ) {
      ...
      if (item.taskType === 10) {
        ...
        for (let channelItem of channelList) {
          ...
          const channelRes = await requestGet(
            "plantChannelNutrientsTask",
            body,
          );
          console.log(`channelRes结果:${JSON.stringify(channelRes)}`);
          if (channelRes && channelRes.code === "0") {
            if (
              channelRes.data && channelRes.data.nutrState &&
              channelRes.data.nutrState === "1"
            ) {
...
function showTaskProcess() {
  return new Promise(async (resolve) => {
    ...
    if ($.taskList && $.taskList.length > 0) {
      ...
      for (let item of $.taskList) {
        console.log(
          `[${item["taskName"]}]  ${item["gainedNum"]}/${item["totalNum"]}   ${
            item["isFinished"]
          }`,
        );
      }
    ...
    if ($.helpResult && $.helpResult.code === "0") {
      ...
      if ($.helpResult.data.helpShareRes) {
        if ($.helpResult.data.helpShareRes.state === "1") {
          ...
          console.log(
            `助力其他情况：${JSON.stringify($.helpResult.data.helpShareRes)}`,
          );
        }
      }
    } else {
...
function showMsg() {
  $.log(`\n${message}\n`);
  jdNotify = $.getdata("jdPlantBeanNotify")
    ? $.getdata("jdPlantBeanNotify")
    : jdNotify;
...
async function plantShareSupportList() {
  ...
  if ($.shareSupportList && $.shareSupportList.code === "0") {
    ...
    data.map((item) => {
      if (
        UTC8_Zero_Time <= item["createTime"] &&
        item["createTime"] < UTC8_End_Time
      ) {
...
function readShareCode() {
  return new Promise(async (resolve) => {
    $.get(
      { url: `http://transfer.nz.lu/bean`, timeout: 10000 },
      (err, resp, data) => {
        try {
          if (err) {
...
function shareCodesFormat() {
  return new Promise(async (resolve) => {
    ...
    if ($.shareCodesArr[$.index - 1]) {
      ...
      const tempIndex = $.index > shareCodes.length
        ? (shareCodes.length - 1)
        : ($.index - 1);
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    const jdPlantBeanShareCodes = $.isNode()
      ? require("./jdPlantBeanShareCodes.js")
      : "";
    ...
    if ($.isNode()) {
      ...
      if ($.getdata("jd_plantbean_inviter")) {
        $.shareCodesArr = $.getdata("jd_plantbean_inviter").split("\n").filter(
          (item) => !!item,
        );
      }
      console.log(
        `\nBoxJs设置的${$.name}好友邀请码:${
          $.getdata("jd_plantbean_inviter")
            ? $.getdata("jd_plantbean_inviter")
            : "暂无"
        }\n`,
      );
...
function requestGet(function_id, body = {}) {
  ...
  return new Promise(async (resolve) => {
    await $.wait(2000);
    const option = {
      url: `${JD_API_HOST}?functionId=${function_id}&body=${
...
function taskUrl(function_id, body) {
  ...
  return {
    url: JD_API_HOST,
    body: `functionId=${function_id}&body=${
      escape(JSON.stringify(body))
