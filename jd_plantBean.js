	...
	if ($.taskList && $.taskList.length > 0) {
		for (let item of $.taskList) {
			...
			if (item.dailyTimes === 1 && item.taskType !== 8) {
				...
				console.log(
					`做 ${item.taskName}任务结果:${JSON.stringify($.receiveNutrientsTaskRes)
					}\n`,
				);
			}
			if (item.taskType === 3) {
...
function requestGet(function_id, body = {}) {
	...
	return new Promise(async (resolve) => {
		await $.wait(2000);
		const option = {
			url: `${JD_API_HOST}?functionId=${function_id}&body=${escape(JSON.stringify(body))
				}&appid=ld`,
			headers: {
				"Cookie": cookie,
				"Host": "api.m.jd.com",
...
function taskUrl(function_id, body) {
	...
	return {
		url: JD_API_HOST,
		body: `functionId=${function_id}&body=${escape(JSON.stringify(body))
